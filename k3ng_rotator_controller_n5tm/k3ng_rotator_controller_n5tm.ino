...
#ifdef FEATURE_SUNF_IC2_DISPLAY
...
byte process_backslash_command(byte input_buffer[], int input_buffer_index, byte source_port, byte include_response_code, char * return_string, byte input_source){
  ...
  switch (input_buffer[1]) {
    ...
    case 'A':      // \Ax[x][x] - manually set azimuth
      ...
      //  ---- hack 6-31-2023 by n5tm to calibrate to .1 degrees Az  This code was coppied from the #else def below.
      place_multiplier = 1;
      ...
      if (decimalplace) {
        new_azimuth = tempfloat / pow(10, (input_buffer_index - decimalplace - 1));
      }
      //  ---- end - hack by n5tm to calibrate to .1 degrees Az  This code was coppied from the #else def below.
      if ((new_azimuth >= 0) && (new_azimuth < 360)) {
        ...
        dtostrf(new_azimuth, 0, 1, temp_string);
        strcat(return_string, temp_string);
      } else {
    ...
    case 'K':          // \Kx   - Force disable the az brake even if a pin is defined (x: 0 = enable, 1 = disable)
      ...
      break;
        ...
        case 'B':      // \Bx[x][x] - manually set elevation
          ...
          if (decimalplace) {
            new_elevation = tempfloat / pow(10, (input_buffer_index - decimalplace - 1));
          }
          //  ---- end - hack by n5tm to calibrate to .1 degrees Az  This code was coppied from the #else def below.
          if ((new_elevation >= 0) && (new_elevation <= 180)) {
            ...
            dtostrf(new_elevation, 0, 1, temp_string);
            strcat(return_string, temp_string);
          } else {
